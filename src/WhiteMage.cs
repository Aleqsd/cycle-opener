...
public static class WhiteMage {
 ...
 public static List<Reminder> Reminders(GuideContext s){
  ...
  if(l>=52)r.Add(new(l>=76?16534u:16531u,"Soins : employer les aptitudes adaptées, puis les Lys selon le besoin. Conserver de quoi répondre aux prochains dégâts.","Groupe / tank",true));
  else if(l>=2)r.Add(new(l>=30?135u:120u,"Soigner avant qu’un équipier soit en danger ; reprendre les dégâts quand le groupe est en sécurité.","Selon les dégâts",true));
  ...
  if(l>=30)r.Add(new(135,"Extra Soin remplace généralement Soin quand un soin direct est nécessaire. Ne pas attendre un effet gratuit.","Soins monocibles",true));
  if(l>=35)r.Add(new(137,"Récup : entretien du tank quand il reçoit des dégâts réguliers.","Soin sur la durée",true));
  if(l>=50)r.Add(new(140,"Bénédiction : remonter rapidement une cible en danger ; préparer son usage sur le tank.","Urgence / soin planifié",true));
  if(l>=52)r.Add(new(3569,"Asile : préparer les soins de zone là où le groupe peut rester.","Groupe / tank",true));
  if(l>=80)r.Add(new(16536,"Tempérance : anticiper les dégâts de groupe importants.","Réduction des dégâts",true));
  if(l>=100)r.Add(new(37011,"Caresse divine : après Tempérance, placer le bouclier avant les prochains dégâts de groupe.","Disponible après Tempérance",true));
  if(l>=12)r.Add(new(125,l>=58?"Vie : utiliser Magie prompte si disponible ; Sponte peut économiser les PM.":l>=18?"Vie : utiliser Magie prompte si disponible et si une résurrection est nécessaire.":"Vie : ressusciter lorsqu’il est possible de terminer l’incantation en sécurité.","Résurrection",true));
  return r;
 }
 ...
 public static List<Step> Opener(GuideContext s){
  ...
  if(l>=92&&s.Targets==1)return [new(f,"Précast avant le pull ; potion au pull si prévue"),new(Dot(l)),new(f),new(f,"Insérer Présence d'esprit après ce sort",Weaves:[136]),new(37009,"Insérer Assises après ce sort",Weaves:[3571]),new(16535,"Lys de sang prêt requis ; sinon utiliser le sort de dégâts habituel"),new(37009),new(37009),new(f),new(f),new(f),new(f),new(f),new(Dot(l),"Renouvellement anticipé sous buffs ; ensuite reprendre les priorités")];
  var r=new List<Step>();
  if(Area(s)){
   r.Add(new(Holy(l),"Quand le tank a regroupé les ennemis"));
   if(l>=30)r.Add(new(Holy(l),l>=56?"Insérer Présence d'esprit puis Assises dans les fenêtres disponibles":"Insérer Présence d'esprit",Weaves:l>=56?[136,3571]:[136]));
   ...
   r.Add(new(f,l>=30?"Insérer Présence d'esprit":"Reprendre les dégâts entre les soins",Weaves:l>=30?[136]:null));
   r.Add(new(f,l>=56?"Insérer Assises":"Répéter selon le besoin",Weaves:l>=56?[3571]:null));
